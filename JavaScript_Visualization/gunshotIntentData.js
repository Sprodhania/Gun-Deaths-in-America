...
if (!gunshotIntentData || gunshotIntentData.length === 0) {
  console.error("No data available for plotting.");
} else {
  // Extract unique years and intents
  let years = [...new Set(gunshotIntentData.map(d => d.year))];
  let intents = [...new Set(gunshotIntentData.map(d => d.intent))];
  ...
  let plotData = intents.map(intent => {
      return {
          x: years,
          y: years.map(year => {
              let record = gunshotIntentData.find(d => d.year === year && d.intent === intent);
              return record ? record.count : 0; // Return 0 if no record is found
          }),
          type: 'scatter', // Chart type
          mode: 'lines+markers', // Marker style
          name: intent // Legend name
      };
  });
